...
//   page 182 of the textbook.
    ...
    for (num = 0; num < N_MESSG ; num++) {
      ...
      // ...
      message->thread_id=which;  //tobe
      message->value=num;  //tobe
...
//      
//----------------------------------------------------------------------
#include <sys/types.h>   //tobe for syscall create
#include <sys/stat.h>   //tobe for syscall create
#include <fcntl.h>   //tobe for syscall create
#include <unistd.h>   //tobe for syscall exit and write
...
ProdCons()
{
    ...
    mutex = new Semaphore("mutex",1);   //tobe
    nfull = new Semaphore("full",0);   //tobe
    nempty = new Semaphore("empty",BUFF_SIZE);   //tobe

    // Put the code to construct a ring buffer object with size 
    //BUFF_SIZE here.
    // ...    

    ring = new Ring(BUFF_SIZE);   //tobe
    ...
    for (i=0; i < N_PROD; i++) 
    {
      ...
      producers[i] = new Thread(prod_names[i]);   //tobe
      producers[i] -> Fork(Producer,i);   //tobe
    };
    ...
    for (i=0; i < N_CONS; i++) 
    {
      ...
      consumers[i] = new Thread(cons_names[i]);   //tobe
      consumers[i] -> Fork(Consumer,i);   //tobe
    };
}
